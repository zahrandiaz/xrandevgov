...
<head>
    ...
    <title>Xrandev - Edit Selector Saran</title>
...
<body class="antialiased bg-gray-100 min-h-screen">
    <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <form method="POST" action="{{ route('suggestion-selectors.update', $suggestionSelector) }}">
                @csrf
                @method('PATCH')
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="title" {{ old('type', $suggestionSelector->type) == 'title' ? 'selected' : '' }}>Judul (Title)</option>
                            <option value="date" {{ old('type', $suggestionSelector->type) == 'date' ? 'selected' : '' }}>Tanggal (Date)</option>
                        </select>
                    </div>

                    <div>
                        <label for="selector" class="block font-medium text-sm text-gray-700">Isi Selector CSS</label>
                        <input type="text" id="selector" name="selector" value="{{ old('selector', $suggestionSelector->selector) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" id="priority" name="priority" value="{{ old('priority', $suggestionSelector->priority) }}" required
                            class="block w-full mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                    </div>
                </div>
